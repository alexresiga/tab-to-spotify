...
function getLatest() {
    $.getJSON('http://tananana.ro/live/meta.php', function (data) {
        ...
        $('div#main').text('');
        $('div#main').append('<div class="card">' + data.current.split(' - ')[1] +'<span id="currently-playing"><i class="fa fa-volume-down" aria-hidden="true"></i></span><br><span>'+ data.current.split(' - ')[0] +'<span></div><br>');
...
function currentyPlaying() {
    ...
    setInterval(currentyPlaying, 2500);
}
